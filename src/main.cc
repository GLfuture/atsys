...
#define LOG_FILE_PATH   "../log/atsys.log"
...
{   
    ...
    else if(pid == 0)
    {
        ...
        api_manager->Register_GET_API("/api/data/card/concrete",std::make_shared<Data_Card_Concrete_API>(data_layer->Get_Mysql_Pool(), data_layer->Get_Cache_Pool()));
        ...
        api_manager->Register_POST_API("/api/smtp",std::make_shared<Smtp_API>(data_layer->Get_Mysql_Pool(), data_layer->Get_Cache_Pool()));
